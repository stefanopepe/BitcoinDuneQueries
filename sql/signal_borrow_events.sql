WITH
stable_tokens AS (
  SELECT contract_address, lower(symbol) AS symbol, decimals
  FROM tokens.erc20
...
morpho_btc_supplies AS (
  SELECT
    ...
    s.evt_block_time AS action_time
  ...
  WHERE s.evt_block_time >= TIMESTAMP '{start_ts}' - INTERVAL '7' DAY
    AND s.evt_block_time < TIMESTAMP '{end_ts}' + INTERVAL '7' DAY
),
cohort_borrows AS (
  SELECT b.*
...
stable_to_btc_swaps AS (
  ...
  JOIN cohort_receivers cr
    ON cr.receiver = lower(to_hex(t.taker))
  JOIN stable_tokens st
    ON st.contract_address = t.token_sold_address
  JOIN btc_tokens bt
    ON bt.contract_address = t.token_bought_address
