...
btc_backed_borrows AS (
  ...
  WHERE EXISTS (
    SELECT 1
    FROM btc_collateral_supplies s
...
SELECT
  ...
  CASE
    WHEN a.all_borrow_events > 0
      THEN COALESCE(c.cohort_borrow_events, 0) / CAST(a.all_borrow_events AS DOUBLE)
    ELSE 0
  END AS event_coverage_share,
  a.all_borrowers,
  COALESCE(c.cohort_borrowers, 0) AS cohort_borrowers,
  CASE
    WHEN a.all_borrowers > 0
      THEN COALESCE(c.cohort_borrowers, 0) / CAST(a.all_borrowers AS DOUBLE)
    ELSE 0
  END AS borrower_coverage_share,
  a.all_borrow_usd_approx,
  COALESCE(c.cohort_borrow_usd_approx, 0) AS cohort_borrow_usd_approx,
  CASE
    WHEN a.all_borrow_usd_approx > 0
      THEN COALESCE(c.cohort_borrow_usd_approx, 0) / a.all_borrow_usd_approx
    ELSE 0
  END AS volume_coverage_share
FROM agg_all a
CROSS JOIN agg_cohort c
